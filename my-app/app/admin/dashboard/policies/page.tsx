...
export default async function PoliciesListPage() {
  ...
  const CAT: Record<string, string> = {
    basics: t.policies.catBasics,
    governance: t.policies.catGovernance,
    guides: t.policies.catGuides,
  };
  const { data, error } = await supabase
    .from("policies")
    .select("id, title_ar, category, version, published")
    .order("sort_order");
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">{t.policies.heading}</h1>
        <Link href="/admin/dashboard/policies/new" className="inline-flex items-center gap-1.5 rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
          <Plus className="size-4" /> {t.policies.newPolicy}
        </Link>
      </div>
          ...
          rows.map((p) => (
            <div key={p.id} className="flex items-center justify-between rounded-xl border p-4">
              <div className="flex items-center gap-3" dir="rtl">
                ...
                <div>
                  <div className="font-medium">{p.title_ar}</div>
                  <div className="text-xs text-muted-foreground">{CAT[p.category] ?? p.category}</div>
                </div>
