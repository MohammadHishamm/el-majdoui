...
export default function MosquesStatsSection() {
  const containerRef = useRef(null);
  const num1Ref = useRef(null);
  const num2Ref = useRef(null);

  const label1Text = "مسجد قائم ومُدار";
  const label1Words = label1Text.split(' ');

  const label2Text = "مبادرات علمية ومجتمعية مفعّلة";
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      mm.add("(min-width: 768px)", () => {
        ...
        // 4. Count up animation for number 1 (15)
        ...
        // 6. Count up animation for number 2 (8)
      ...
      mm.add("(max-width: 767px)", () => {
        ...
        // 2. Count up animation for number 1 (15)
        const count1Obj = { val: 0 };
        tl.to(count1Obj, {
          val: 15,
          duration: 1.0,
        ...
        // 4. Count up animation for number 2 (8)
        const count2Obj = { val: 0 };
        tl.to(count2Obj, {
          val: 8,
          duration: 1.2,
      ...
      <div className="animate-section-content-s3" style={{ width: '100%' }}>
        <div className={styles.row}>
            ...
            <div className={styles.statCard}>
              ...
              <span className={styles.statLabel}>
                {label1Words.map((word, idx) => (
                  ...
                  >
                    {word}
                    {idx < label1Words.length - 1 ? '\xa0' : ''}
                  </span>
                ))}
              </span>
            </div>

            {/* Card 2: 8 مبادرات علمية ومجتمعية مفعّلة */}
            <div className={styles.statCard}>
          ...
          <div className={`${styles.imageWrapper} animate-stat-img-wrapper`}>
            <Image
              src="/images/focus-area/masged-majdoui/section-2.png"
              alt="مسجد من مساجد المجدوعي"
              fill
